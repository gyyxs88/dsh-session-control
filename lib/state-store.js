...
function parentRollup(children) {
  if (children.length === 0) {
    return { status: 'prepared', attention: null, childIds: [] }
  }
...
export class OperationStore {
  ...
  recentCount(sourceId, targetId, sinceMs) {
    return this.list({ sourceId, targetId }).filter((row) => (
      row.kind !== 'batch' && Date.parse(row.createdAt) >= sinceMs
    )).length
  }
  ...
  canInsert(count = 1) {
    ...
    const roots = this.list()
      .filter((row) => row.parentId === null && TERMINAL_OPERATION_STATUSES.has(row.status))
      .toReversed()
  ...
  rollupParent(parentId) {
    ...
    const rollup = parentRollup(children)
