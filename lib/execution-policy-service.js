...
function currentPermission(ctx, agent) {
  ...
  const permission = ctx.permissionPresets.current(sessionEvents(agent.session))
  if (!PERMISSIONS.has(permission)) policyError('target Session permission preset is unsupported')
  return permission
}

function effectivePermission(current, requested) {
  if (requested === undefined) return current
  if (!PERMISSIONS.has(requested)) policyError('requested child execution permission is unsupported')
  if (PERMISSION_RANK[requested] > PERMISSION_RANK[current]) {
    policyError('requested child execution permission exceeds the target Session', 'EXECUTION_POLICY_ELEVATION_REQUIRED')
  }
  return requested
}
...
export function createSessionControlExecutionPolicyServices({ ctx, controllerSessionIds = [], approvalTimeoutMs = 30_000 } = {}) {
  ...
  const controllers = controllerSessionIds instanceof Set
    ? controllerSessionIds
    : new Set(controllerSessionIds)
  const resolve = async ({ exec, request } = {}) => {
    ...
    const targetPermission = currentPermission(ctx, target)
    const permission = effectivePermission(targetPermission, request?.executionPermission)
  ...
  const verifier = {
    async verifyTargetSessionPolicy({ exec, policy } = {}) {
      ...
      const targetPermission = currentPermission(ctx, target)
      const permission = effectivePermission(targetPermission, policy.permission)
      const workspaceRoot = target.session?.header?.cwd ?? target.session?.meta?.cwd
      if (workspaceRoot !== policy.workspaceRoot) policyError('execution policy changed before channel launch', 'EXECUTION_POLICY_STALE')
      return {
        verified: true,
