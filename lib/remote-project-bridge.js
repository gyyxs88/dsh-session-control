...
export function createRemoteProjectPort({ api, ctx, hostId, sourceAllowlist = [], logger = console } = {}) {
  ...
  const permissionFor = (agent, label) => {
    ...
    const preset = ctx.permissionPresets.current(agent.session?.events ?? [])
  ...
  return {
    ...
    async verifyTargetSessionPolicy(frame) {
      ...
      if (frame.request?.permission !== undefined && frame.request.permission !== permission) throw new Error('requested execution policy does not match target Session')
