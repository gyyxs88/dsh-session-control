...
export function validateRemoteProjectManifest(manifest = REMOTE_PROJECT_MANIFEST) {
  ...
  if (!Array.isArray(manifest.capabilities) || ['session-control.port', 'remote-project.open', 'remote-project.runtime-auth-begin', 'remote-project.runtime-auth-confirm', 'remote-project.execution-policy-verify'].some((capability) => !manifest.capabilities.includes(capability))) throw new Error('remote project capabilities are invalid')
