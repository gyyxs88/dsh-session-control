...
function registerControllerTools(toolCtx, api, store) {
  ...
  register({
    name: 'session_events',
    description: '读取一个已授权 live 或 cold 普通会话的显著事件，且不会为读取而恢复 cold 会话。默认只返回类型和坐标；正文或工具结果内容会额外触发人类审批。',
    parameters: {
  ...
  register({
    name: 'session_send',
    description: '经人类逐次审批，向一个已授权 live 普通会话投递不受信的委派任务。必须提供稳定幂等键；重复调用不会重复投递。返回 operation_id，随后用 session_wait。',
    parameters: {
      target_id: { type: 'string', required: true, description: '目标会话 id' },
  ...
  register({
    ...
    description: '经一次知情审批向 1–8 个授权范围内 live 普通会话分发任务，创建一个持久 batch 父 operation 和精确子 operation。整个批次及每个子项都必须使用稳定且互不相同的幂等键。',
    parameters: {
  ...
  register({
    ...
    description: '经人类逐次审批，中断一个已授权 live 普通会话当前轮，并保留尚未领取的 inbox。可等待其真正回到 idle。',
    parameters: {
      target_id: { type: 'string', required: true, description: '目标会话 id' },
  ...
  register({
    name: 'session_open',
    description: '经人类逐次审批创建、恢复或 fork 普通会话。create 继承控制会话 preset/model；resume 使用持久 session；fork 只复制到完整 turn 边界。新会话由插件生命周期持有，不创建 Git worktree。',
    parameters: {
  ...
  register({
    ...
    description: '经人类逐次审批，在目标普通会话中创建原生持久定时任务。目标为 cold 时会临时恢复、持久化任务后重新收敛为 cold；到期时控制插件自动恢复并由 DSH Schedule 投递。',
    parameters: {
      target_id: { type: 'string', required: true },
  ...
  register({
    ...
    description: '列出目标 live 或 cold 普通会话的原生持久定时任务。默认隐藏提醒正文，只返回正文哈希；include_prompt=true 需人类审批。',
    parameters: {
      target_id: { type: 'string', required: true },
  ...
  register({
    ...
    description: '经人类逐次审批，删除目标会话中的一个原生持久定时任务。cold 目标会临时恢复以完成持久事务。',
    parameters: {
      target_id: { type: 'string', required: true },
  ...
  register({
    ...
    description: '经人类逐次审批，确保绝对目录存在并注册为 DSH Workspace；相同规范路径幂等复用现有 Workspace。',
    parameters: {
  ...
  register({
    ...
    description: '经一次知情审批完成项目启动链：确保目录存在、注册/复用 DSH Workspace、通过 Core Agent Factory 创建会话并 attach 到侧栏工作区。',
    parameters: {
