...
function makeApi(ctx, config, store, controllers, lifecycle = {
  ...
  async function send(args, exec, { parentId = null, skipCapacity = false } = {}) {
    ...
    const operation = {
      ...
      sourceTitle: sessionDisplayTitle(source.session.events) ?? null,
      targetId: target.id,
      targetCwd: target.session.header.cwd,
      targetTitle: sessionDisplayTitle(target.session.events) ?? null,
      idempotencyKey: key,
      contentHash: hash,
      messageId: '',
      status: 'prepared',
  ...
  async function getPermission(args, exec) {
    ...
    const view = await resolveSessionView(source, String(args.target_id ?? ''))
    if (view.agent !== undefined) await ctx.sessions.flush(view.agent.session)
    const events = view.agent === undefined ? view.events : view.agent.session.events
    return {
      ok: true,
      target_id: view.id,
  ...
  async function setPermission(args, exec, { parentId = null, targetAgent } = {}) {
    ...
    let sourcePreset = ctx.permissionPresets.current(source.session.events)
    ...
    let view = targetAgent === undefined
      ...
      : {
        ...
        events: targetAgent.session.events,
      }
    ...
    let previous = permissionSummary(view.agent === undefined ? view.events : view.agent.session.events)
      .permission_preset
    const now = new Date().toISOString()
    let operation = await store.add({
      id: randomUUID(),
    ...
    try {
      ...
      sourcePreset = ctx.permissionPresets.current(source.session.events)
      if (!['workspace-write', 'danger-full-access'].includes(sourcePreset)) {
        throw new Error('控制会话在执行前已降级，权限调整取消')
      }
      previous = ctx.permissionPresets.current(target.session.events)
      operation = await store.update(operation.id, {
        previousPermissionPreset: previous,
        targetCwd: target.session.header.cwd,
      })
      ...
      applied = true
      await ctx.sessions.flush(target.session)
      const actual = ctx.permissionPresets.current(target.session.events)
  ...
  async function initializePermission(source, target, args, exec, parentOperation) {
    ...
    const sourcePreset = ctx.permissionPresets.current(source.session.events)
    const current = ctx.permissionPresets.current(target.session.events)
    ...
    if (ctx.permissionPresets.current(target.session.events) === requested) return null
  ...
  async function handlePermissionPreStep(payload, next) {
    ...
    for (const message of messages) {
      ...
      let applied = false
      try {
        const current = ctx.permissionPresets.current(payload.agent.session.events)
        ...
        const sourcePreset = source === undefined
          ? 'offline'
          : ctx.permissionPresets.current(source.session.events)
        if (!['workspace-write', 'danger-full-access'].includes(sourcePreset)) {
          operation = await store.update(operation.id, {
            status: 'failed',
        ...
        const freshSourcePreset = freshSource === undefined
          ? 'offline'
          : ctx.permissionPresets.current(freshSource.session.events)
        const freshTargetPreset = ctx.permissionPresets.current(payload.agent.session.events)
        ...
        const actual = ctx.permissionPresets.current(payload.agent.session.events)
        ...
        return api.resolveSessionView(exec.agent, targetId)
          .then((view) => {
            ...
            const current = ctx.permissionPresets.current(
              view.agent === undefined ? view.events : view.agent.session.events,
            )
            if (!['workspace-write', 'danger-full-access'].includes(sourcePreset)) {
