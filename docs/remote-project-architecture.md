...
- 正式 `dsh-session-control` Unix bridge 暴露 `remote-project.ping`、`remote-project.open`、`remote-project.schedule-delete`、`remote-project.runtime-auth-begin`、`remote-project.runtime-auth-confirm` 和 `remote-project.execution-policy-verify`；来源 `sourceHostId/sourceSessionId` 只能命中 Host 配置的 capability allowlist，并映射到远端实际执行官方 API 的 `controllerSessionId`，不能拿跨 Host 来源 ID 查询远端 Agent。Schedule 删除继续调用官方 `deleteSchedule` 并以独立幂等 operation 持久化，不允许跨主机控制面直接改 Session 日志。
